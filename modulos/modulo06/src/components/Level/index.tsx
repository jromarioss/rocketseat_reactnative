...
export function Level({ title, type = 'EASY', isChecked = false, ...rest }: Props) {
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    return {
      transform: [{ scale: scale.value }], //animação na scale o value é o valor
  ...
  function onPressIn() {
    scale.value = withTiming(1.1) //altera o tamanho aumenta, qnd clica no btn
  }

  function onPressOut() {
    scale.value = withTiming(1); //qnd solta o btn
  }
