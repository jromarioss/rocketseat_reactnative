...
const CHECK_SIZE = 28; //tamanho
const CHECK_STROKE = 2; //espesura

export function Option({ checked, title, ...rest }: Props) {
  const percentage = useValue(0)
  const circle = useValue(0)

  const RADIUS = (CHECK_SIZE - CHECK_STROKE) / 2; //o raio
  const CENTER_CIRCLE = RADIUS / 2;

  const path = Skia.Path.Make(); //cria um caminho como se fosse uma canet desenhado
  path.addCircle(CHECK_SIZE, CHECK_SIZE, RADIUS)

  useEffect(() => {
    if (checked) {
      runTiming(percentage, 1, { duration: 700 }) //chama direto o runTiming, 1pr a variável, 2pr o valor, 3 um evento
